...
app.use((req, res, next) => {
    ...
    if (req.body && Object.keys(req.body).length > 0) {
        ...
        if (bodyLog.newPassword)
            bodyLog.newPassword = '***HIDDEN***';
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        const existing = await userModel_1.default.findOne({ $or: [{ username }, { email }] });
        if (existing) {
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ username: user.username, email: user.email, role: user.role }, JWT_SECRET, { expiresIn: '1d' });
...
app.post('/api/forgot-password', async (req, res) => {
    ...
    try {
        ...
        // Sauvegarde en BDD
        await userModel_1.default.findByIdAndUpdate(user._id, { resetToken, resetTokenExpiration });
...
app.post('/api/reset-password', async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            ...
            const allTokens = await userModel_1.default.find({ resetToken: { $ne: null } });
...
app.use('/api', logRouteUsage('AUTH'), authRoutes_1.default);
console.log('✅ Routes AUTH chargées sur /api');
app.use('/api', logRouteUsage('QUIZ'), quizRoutes_1.default);
console.log('✅ Routes QUIZ chargées sur /api');
app.use('/api/ranking', logRouteUsage('SCORE'), scoreRoutes_1.default);
console.log('✅ Routes SCORE chargées sur /api/ranking');
app.use('/api/profile', logRouteUsage('PROFILE'), profileRoutes_1.default);
console.log('✅ Routes PROFILE chargées sur /api/profile');
app.use('/api/admin', logRouteUsage('ADMIN'), adminRoutes_1.default);
