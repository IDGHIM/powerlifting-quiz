...
router.get('/quiz', async (req, res) => {
    ...
    try {
        ...
        const questions = await questionModel_1.Question.find({
...
router.get('/quiz/random', async (req, res) => {
    ...
    try {
        ...
        const questions = await questionModel_1.Question.aggregate([
            { $match: query },
            { $sample: { size: limit } }
        ]);
...
router.get('/quiz/categories', async (req, res) => {
    ...
    try {
        const categories = await questionModel_1.Question.distinct('category');
...
router.get('/quiz/count', async (req, res) => {
    ...
    try {
        const counts = await questionModel_1.Question.aggregate([
            {
                $group: {
                    _id: '$category',
                    count: { $sum: 1 }
...
router.get('/quiz/:id', async (req, res) => {
    ...
    try {
        const question = await questionModel_1.Question.findById(req.params.id);
        if (!question) {
